...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', rotate=False):
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=custom_dataset_class.collate_fn4 if (quad and not rotate) else custom_dataset_class.collate_fn)
...
def rotate_random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0), mosaic=False):
    """
        torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
        targets = [cls, xyxyxyxy]
    ...
    img, targets_xyxyxyxy = polygon_random_perspective(img, targets_xyxyxyxy, segments, degrees, translate, scale, shear, perspective,
                                       border, mosaic)
...
class Rotate_LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        desc = f"{prefix}Scanning '{path.parent / path.stem}' images and labels..."
        with Pool(num_threads) as pool:
            pbar = tqdm(pool.imap_unordered(rotate_verify_image_label, zip(self.img_files, self.label_files, repeat(prefix))),
                        desc=desc, total=len(self.img_files))
            for im_file, l, shape, segments, nm_f, nf_f, ne_f, nc_f in pbar:
                nm += nm_f
                nf += nf_f
