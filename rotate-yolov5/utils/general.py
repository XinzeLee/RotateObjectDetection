...
def labels_to_class_weights(labels, nc=80):
    ...
    labels = np.concatenate(labels, 0)  # labels.shape = (866643, 5) for COCO, labels.shape = (866643, 7) for rotate COCO
    classes = labels[:, 0].astype(np.int)  # labels = [class xywh], labels = [class, xywhrm] for rotate
...
# rotate does not support a second stage classifier
